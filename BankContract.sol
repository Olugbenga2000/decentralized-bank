...
contract SmartBankAccount {
    ...
    function addBalanceERC20(address erc20TokenSmartContractAddress) public {
        ...
        path[0] = token;
        path[1] = uniswap.WETH();
        uint256[] memory amountOut = uniswap.swapExactTokensForETH(approvedAmountOfERC20Tokens, amountETHMin, path, to, deadline);
        //TODO : rest of the logic
        // 3. deposit eth to compound
        uint256 cEthOfContractBeforeMinting = ceth.balanceOf(address(this)); //this refers to the current contract
        
        // send ethers to mint()
        ceth.mint{value: amountOut[1]}();
    ...
    function withdraw() public payable {
     ...
     function withdrawERC20(address erc20TokenSmartContractAddress) public {
        IERC20 erc20 = IERC20(erc20TokenSmartContractAddress);
        address payable withdrawTo = payable(msg.sender);
        uint amountToTransfer = getBalance(msg.sender);
        ceth.redeem(balances[msg.sender]);
        ...
        uint256[] memory amountOut = uniswap.swapExactETHForTokens{value : amountToTransfer}(amountOutMin, path, address(this), deadline);
        balances[msg.sender] = 0;
        erc20.transfer(withdrawTo, amountOut[1]);
     }
